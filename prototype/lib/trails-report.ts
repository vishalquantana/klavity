...
export interface WalkReportData {
  ...
  replayUrl?: string
  /** Per-project agency branding (KLAVITYKLA-223). Resolved to render-ready defaults; unbranded projects render as before. */
...
export async function gatherWalkReport(
  ...
  // KLAVITYKLA-223: per-project agency branding. Best-effort — unbranded default on any miss.
...
export function renderWalkReportHtml(
  ...
  // KLAVITYKLA-223: agency branding — logo + accent skin the header, and the footer carries the
  // "Monitored by <Agency> · powered by Klavity" PLG line (or agency-only under white-label).
  // Unbranded projects fall back to Klavity's own default accent + wordmark, rendering as before.
...
<!-- Cover header (agency-branded when configured) -->
<div style="padding:24px 0 20px;border-bottom:2px solid ${esc(accent)};margin-bottom:28px;display:flex;align-items:center;gap:12px">
  ${branding.logoDataUrl
    ? `<img src="${esc(branding.logoDataUrl)}" alt="${esc(branding.name || "Agency")} logo" style="height:32px;max-width:140px;border-radius:8px;display:inline-block;vertical-align:middle;object-fit:contain" />`
    : wordmarkSvg}
  <div>
    <div style="font-family:'Fraunces',Georgia,serif;font-size:20px;font-weight:600;letter-spacing:-.02em">${esc(branding.name || "Klavity")}</div>
...
<blockquote style="margin-bottom:24px;border-left:3px solid ${esc(accent)};padding:10px 16px;background:#f5f5ff;border-radius:0 8px 8px 0;color:#374151;font-size:14px;font-style:italic">
...
<div style="border-top:1px solid #e5e7eb;padding-top:14px;margin-top:8px;display:flex;justify-content:space-between;align-items:center;font-size:11px;color:#9ca3af;font-family:monospace">
  <span>${brandingFooterHtml(branding, { signupUrl: `${base}/signup`, linkColor: accent })}</span>
