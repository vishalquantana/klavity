...
//
// The metered metrics and their PLAN_QUOTAS key. SINGLE SOURCE OF TRUTH for the metric → quota
// mapping — lib/quota.ts (enforcement) re-exports this rather than keeping its own copy, so the
// meter a customer sees and the limit enforcement compares against can never drift apart.
export type QuotaMetric = "sim_review" | "autosim_walk"

export const METRIC_TO_QUOTA_KEY: Record<QuotaMetric, keyof typeof PLAN_QUOTAS[keyof typeof PLAN_QUOTAS]> = {
  sim_review: "simReactionsMonthly",
  // KLAVITYKLA-359: autosim_walk is a MONTHLY RUN count — goes against autosimRunsMonthly, not
  // autosimFlows (which is the configured-flow stock, not a monthly run cap).
...
export function buildUsageMeters(plan: string | null | undefined, usage: Record<string, number> = {}): UsageMeterView[] {
  ...
  // Limits are read through METRIC_TO_QUOTA_KEY — the same mapping lib/quota.ts enforces with.
  const defs: Array<{ key: UsageMeterView["key"]; metric: QuotaMetric; label: string; limit: number | null }> = [
    { key: "sims", metric: "sim_review", label: "Sim reviews", limit: (q as any)[METRIC_TO_QUOTA_KEY.sim_review] ?? null },
    ...
    { key: "autosim", metric: "autosim_walk", label: "AutoSim runs", limit: (q as any)[METRIC_TO_QUOTA_KEY.autosim_walk] ?? null },
  ]
  return defs.map((d) => {
    ...
    // MUST match checkQuota() in lib/quota.ts, which degrades at `usage >= limit`. Using `>` here
    // would show an account sitting exactly AT its allowance a calm meter while enforcement is
    // already degrading it (KLAVITYKLA-309 follow-up).
