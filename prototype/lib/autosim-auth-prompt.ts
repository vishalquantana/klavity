...
function buildMintLinkPrompt(base: string[], input: AuthPromptInput, curl: string): string {
  const prompt = [
    ...
    "   - Token format: signed payload with `exp`, `jti`, and project audience, plus HMAC-SHA256 signature. Verify with constant-time compare (`crypto.timingSafeEqual` in Node, or equivalent).",
    ...
    "- `PLACEHOLDER_METHOD` → `" + input.method + "`",
    `- ${input.testEmail} is your Sim test email — paste verbatim.`,
    "  - `PLACEHOLDER_SECRET` → the Klavity-provided opaque `amlt_...` token, or a same-origin path `/test-login?token=amlt_...`. Never paste an absolute URL.",
    "",
