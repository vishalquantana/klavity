...
// nothing else catches a missing import here.
...
async function findDuplicateFeedback(args: {
  ...
  // traits → specific keys → floor 0 (unconditional merge), so recurrence/regression detection is unchanged.
  const exactMinSim = (args.citedTraitIds && args.citedTraitIds.length > 0) ? 0 : 0.3
  const target = chooseDedup({ title: args.title, observation: args.observation }, exact, recent, 0.82, new Set(), exactMinSim)
...
async function quotaExceeded(accountId: string, kind: "projects" | "sims" | "autosimFlows", count: () => Promise<number>): Promise<{ error: string; code: "quota_exceeded" | "past_due"; upgradeUrl: string } | null> {
  ...
  // KLAVITYKLA-365: AutoSim is no longer part of Free (free.autosimFlows = 0), so the generic
  // "your plan includes 0 AutoSim flows" phrasing reads badly. Say it plainly instead. This gates
  // CREATING a new flow only — existing flows keep running (nothing on the walk path calls this).
  if (kind === "autosimFlows" && limit === 0) {
    return {
      error: "AutoSim is available on paid plans — upgrade to guard a flow.",
      code: "quota_exceeded",
      upgradeUrl: "/dashboard?upgrade=pro",
    }
  }
  const noun = kind === "projects" ? (limit === 1 ? "project" : "projects")
    : kind === "autosimFlows" ? (limit === 1 ? "AutoSim flow" : "AutoSim flows")
    : (limit === 1 ? "Sim" : "Sims")
  return {
...
// KLAVITYKLA-524: opts.advanceStatusOnSuccess advances the feedback row 'new' → 'open' once an
// external ticket is SUCCESSFULLY created, so an autofiled human Snap leaves "New Reports" and appears
...
function autoCopyFeedback(feedbackId: string, projectId: string, actor: string | null, effectivePriority?: string | null, opts?: { advanceStatusOnSuccess?: boolean; trustedProvenance?: boolean }): void {
  void (async () => {
    try {
      ...
      // "New Reports" into the Tickets list. Guarded on (a) an actual create success this run and
      // (b) the caller opting in (autofile-on-submit only). advanceFeedbackToOpenIfNew is itself
      // WHERE status='new', so it's a no-op if the row was already advanced/triaged (idempotent) and
      // never downgrades an urgent/high report that started 'open'.
      if (opts?.advanceStatusOnSuccess && anyExportSucceeded) {
        await advanceFeedbackToOpenIfNew(feedbackId, projectId)
          .catch((e: any) => console.warn("[snap-routing] status advance failed (non-fatal):", e?.message || e))
      }
    } catch (err: any) {
...
async function handle(req: Request, server: { requestIP?: (r: Request) => { address?: string } | null }): Promise<Response> {
    ...
    // ── transcripts → preview (Task 6) — compute enrichment ops WITHOUT persisting; stash for /apply ──
    // Mirrors the auth/project guards of POST /api/transcripts exactly. Only DB write is the stash
    // (insertPendingTranscript) — no transcripts/sim_traits/trait_events writes here.
    if (req.method === "POST" && path === "/api/transcripts/preview") {
      ...
      try {
        ...
        for (const p of personas) {
          if (p.simClass !== "client" && p.type !== "client") { gi++; continue } // only client speakers become/enrich Sims
          const match = matchPersonaToSim({ name: String(p?.name || ""), role: p?.role ? String(p.role) : undefined }, sims)
          if (match && "simId" in match) {
            ...
            const opsOut = ops.map((o, oi) => {
              ...
              return {
                opId: `${gi}:${oi}`, op: o.op, kind: o.kind, text: o.text, quote: g.quote,
            ...
            stash.push({ kind: "existing", groupIndex: gi, simId: match.simId, ops })
          } else {
            const insights = Array.isArray(p.insights) ? p.insights : []
            const insightsOut = insights.map((ins: any, oi: number) => {
            ...
            groups.push({ kind: "new", proposalId: `new_${gi}`, proposedName: p.name, role: p.role, insights: insightsOut })
            stash.push({ kind: "new", groupIndex: gi, proposalId: `new_${gi}`, persona: p })
          }
          gi++
        }
...
<a href="/" style="display:inline-block;background:#4f46e5;color:#fff;text-decoration:none;font-size:14px;font-weight:600;padding:10px 24px;border-radius:8px">Return to Klavity</a>
    ...
    if (path === "/api/trails/dashboard" || path === "/api/trails/trends" || path === "/api/trails/walks" || path.startsWith("/api/trails/findings/") || path.startsWith("/api/trails/walks/")
      ...
      if (req.method === "POST" && path === "/api/trails/author") {
        ...
        // (runAuthorNow → lib/trails-crystallize.ts createTrail), so gating here gates creation
        // without touching any run/schedule path — existing flows are unaffected by construction.
        // Checked BEFORE the expensive author drive starts. Dark unless KLAV_BILLING_ENFORCEMENT=1.
        {
          const acctId = await accountIdForProject(projectId)
          if (acctId) {
            const flowQuota = await quotaExceeded(acctId, "autosimFlows", () => countAccountAutosimFlows(acctId))
            if (flowQuota) return json(flowQuota, 402)
          }
    ...
    // GET /api/projects/:id/reported-pages — the distinct pages that have at least one report,
    // powering the dashboard "Bugs by page" picker for teammates who don't use the QA overlay.
    // Placed here (above the generic /api/ login gate, a SIBLING of — not inside — the qa-mode
    // block) so an unauthenticated call gets a clean JSON 401 rather than an HTML /login redirect,
    // exactly like the page-bugs / qa-close endpoints it pairs with. Team-gated by projectAccess.
    // Deliberately OUTSIDE the qa-mode / transcribe / connector route regions.
    {
      const reportedPagesMatch = path.match(/^\/api\/projects\/([^/]+)\/reported-pages$/)
      if (req.method === "GET" && reportedPagesMatch) {
        const meRP = (await sessionEmail(req)) || (await bearerEmail(req))
        if (!meRP) return json({ error: "Sign in to continue." }, 401)
        const projectId = reportedPagesMatch[1]
        const access = await projectAccess(meRP, projectId)
        if (!access) return json({ error: "You are not a member of this project." }, 403)
        if (!rlAllow(`qa:pages:${meRP}`, 120, 60 * 1000)) return json({ error: "Too many requests." }, 429)
        const rows = await distinctReportedPages(projectId, 200)
        const pages = rows
          .map((r) => ({
            url: r.urlHost ? `https://${r.urlHost}${r.urlPath || ""}` : (r.urlPath || ""),
            host: r.urlHost,
            path: r.urlPath,
            count: r.count,
            lastAt: r.lastAt,
    ...
    if (path.startsWith("/api/")) {
      ...
      // accounts.name is the workspace display name (surfaced via /api/me active.name). Admin/owner
      // only. Trimmed + length-capped so a blank or oversized name can't be persisted.
      if (req.method === "PATCH" && path === "/api/account") {
        ...
        if (!clean) return json({ error: "Workspace name can't be empty." }, 400)
        if (clean.length > 80) return json({ error: "Workspace name is too long (max 80)." }, 400)
        await renameAccount(active.workspaceId, clean)
      ...
      if (projMatch) {
        ...
        // ── Developer / API tokens management (wip/ci-token-settings) ─────────────────────────────
        // Session-gated + project-ADMIN-only management of the kci_ CI tokens used to trigger AutoSims
        // via REST (/api/v1/*) and MCP (/mcp). Tokens are stored HASHED, so the plaintext is returned
        // ONCE only (copy-once). All three routes are admin-gated + strictly project-scoped (IDOR).
        //   GET    /api/projects/:id/ci-tokens            → { tokens:[metadata…] }  (never the token/hash)
        //   POST   /api/projects/:id/ci-tokens  {name}    → 201 { id,name,prefix,token }  (token ONCE)
        //   DELETE /api/projects/:id/ci-tokens/:tokenId   → revoke by id, scoped to THIS project
        if (sub === "/ci-tokens" || sub.startsWith("/ci-tokens/")) {
          if (!me) return json({ error: "Sign in to continue." }, 401)
          if (access !== "admin") return json({ error: "Only project admins can manage API tokens." }, 403)

          if (req.method === "GET" && sub === "/ci-tokens") {
            const tokens = await listCITokens(pid)
            return json({ tokens })
          }

          if (req.method === "POST" && sub === "/ci-tokens") {
            // Rate-limit minting per project+admin so a compromised session can't flood-mint tokens.
            if (!rlAllow(`citoken:mint:${pid}:${me}`, 10, 60 * 60 * 1000)) return json({ error: "Too many tokens minted. Try again later." }, 429)
            const body = await req.json().catch(() => ({}))
            const name = String(body.name || "").trim().slice(0, 80)
            if (!name) return json({ error: "A token name is required." }, 400)
            const { id, token, prefix } = await issueCITokenNamed(me, pid, name)
            // The FULL token appears here and NOWHERE else (no log line, never in GET). Copy-once.
            return json({ id, name, prefix, token }, 201)
          }

          const delMatch = sub.match(/^\/ci-tokens\/([^/]+)$/)
          if (req.method === "DELETE" && delMatch) {
            const tokenId = delMatch[1]
            // IDOR guard: revokeCITokenById scopes the UPDATE to `pid`, so a token id from another
            // project matches zero rows → false → 404 here (never a cross-project revoke).
            const ok = await revokeCITokenById(pid, tokenId)
